...
var PROTOCOL_LIST = []string{"tcp", "udp", "kcp", "quic"}
...
const (
	...
	DEFAULT_KCP_BLKSIZE  = 4 * 1024   // default read/write block size for KCP
	DEFAULT_QUIC_BLKSIZE = 64 * 1024  // default read/write block size for QUIC
	TCP_MSS              = 1460       // tcp mss size
	KCP_MSS              = 1376       // kcp mss size
	// kcp / quic
	DEFAULT_WRITE_BUF_SIZE = 4 * 1024 * 1024 // write buffer size
	DEFAULT_READ_BUF_SIZE  = 4 * 1024 * 1024 // read buffer size
	DEFAULT_FLUSH_INTERVAL = 10              // flush interval 10 ms default
	MS_TO_NS               = 1000000
	S_TO_NS                = 1000000000
	MB_TO_B                = 1024 * 1024
	KB_TO_B                = 1024
...
const (
	...
	KCP_INTERVAL_HEADER      = "[ ID]    Interval        Transfer        Bandwidth        RTT        Retrans   Retrans(%%)  Lost(%%)  Early(%%)  Fast(%%)\n"
	KCP_RESULT_HEADER        = "[ ID]    Interval        Transfer        Bandwidth        RTT        Retrans   Retrans(%%)  Lost(%%)  Early(%%)  Fast(%%)  Recover(%%)  PktsLost(%%)  SegsLost(%%)\n"
	TCP_REPORT_SINGLE_STREAM = "[  %v] %4.2f-%4.2f sec\t%5.2f MB\t%5.2f Mb/s\t%6.1fms\t%4v\n"
	KCP_REPORT_SINGLE_STREAM = "[  %v] %4.2f-%4.2f sec\t%5.2f MB\t%5.2f Mb/s\t%6.1fms\t%4v\t%2.2f%%\t%2.2f%%\t%2.2f%%\t%2.2f%%\n"
	TCP_REPORT_SINGLE_RESULT = "[  %v] %4.2f-%4.2f sec\t%5.2f MB\t%5.2f Mb/s\t%6.1fms\t%4v\t%2.2f%%\t[%s]\n"
	KCP_REPORT_SINGLE_RESULT = "[  %v] %4.2f-%4.2f sec\t%5.2f MB\t%5.2f Mb/s\t%6.1fms\t%4v\t%2.2f%%\t%2.2f%%\t%2.2f%%\t%2.2f%%\t%2.2f%%\t%2.2f%%\t%2.2f%%\t[%s]\n"
...
type iperf_setting struct {
	...
	// protocol-specific options (for KCP, QUIC, etc.)
	snd_wnd        uint
	rcv_wnd        uint
