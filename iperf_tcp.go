...
func (tcp *tcp_proto) init(test *iperf_test) int {
	for _, sp := range test.streams {
		// 设置socket读写缓冲区大小，这对高带宽网络至关重要
		err := sp.conn.(*net.TCPConn).SetReadBuffer(int(test.setting.read_buf_size))
		if err != nil {
			log.Debugf("Failed to set TCP read buffer: %v", err)
		}
		err = sp.conn.(*net.TCPConn).SetWriteBuffer(int(test.setting.write_buf_size))
		if err != nil {
			log.Debugf("Failed to set TCP write buffer: %v", err)
		}
